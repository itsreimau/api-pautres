  ...
  switch ($api) {
    case "vihangayt":
      ...
      foreach ($instances as $instance) {
        $api_url =
          "https://vihangayt.me/tools/chatgpt$instance?q=" . urlencode($query);
        ...
        if (
          ...
          isset($chatgpt_response["data"]["message"])
        ) {
    ...
    case "apinepdev":
      $api_url =
        "https://chatgpt.apinepdev.workers.dev/?question=" . urlencode($query);
      $response = file_get_contents($api_url);
      $chatgpt_response = json_decode($response, true);
...
// Tell the user JSON data is incomplete
  ...
  echo json_encode([
    "replies" => [
      ...
      [
        "message" =>
          "JSON data is incomplete. Was the request sent by AutoResponder?",
      ],
    ],
  ]);
}
