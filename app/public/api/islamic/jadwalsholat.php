...
{
    $api_url = "https://api-gabut.bohr.io/api/prayerschedulenow?query=" . urlencode($message);
...
if (!empty($data->query) && !empty($data->appPackageName) && !empty($data->messengerPackageName) && !empty($data->query->sender) && !empty($data->query->message)) {
    ...
    $defaultMessage .= "➲ Imsak: %imsak%\n";
    $defaultMessage .= "➲ Subuh: %subuh%\n";
    $defaultMessage .= "➲ Terbit: %terbit%\n";
    $defaultMessage .= "➲ Dhuha: %dhuha%\n";
    $defaultMessage .= "➲ Dzuhur: %dzuhur%\n";
    $defaultMessage .= "➲ Ashar: %ashar%\n";
